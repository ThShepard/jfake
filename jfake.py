...
parser.add_argument(
    ...
    "-i",
    required=True,
    help="Input image file [BMP], [GIF], [JPEG], [PNG], [PPM], [TIFF] (required)",
)
parser.add_argument(
    "--output",
    "-o",
...
parser.add_argument(
    ...
    action="store_true",
    dest="verbose",
    help="Write all steps to terminal (default: False)",
)
parser.add_argument(
    "--debug",
    "-d",
    action="store_true",
    dest="debug",
    help="Write all steps to output folder (default: False)",
)
parser.add_argument(
    "--quality",
    "-q",
...
parser.add_argument(
    ...
    action="store_true",
    dest="benchmark",
    help="Write needed time per step in file (default: False)",
)
parser.add_argument(
    "--entropy",
    "-e",
    action="store_true",
    dest="entropy",
    help="Calculate entropy in each processing step (default: False)",
)
parser.add_argument(
    "--psnr",
    "-p",
    action="store_true",
    dest="psnr",
    help="Calculate signal-to-noise-ratio (PSNR) (default: False)",
)
parser.add_argument(
    "--numba",
    "-n",
    action="store_true",
    dest="numba",
    help="Use numba jit compiler for better performance (default: False)",
)
parser.add_argument(
    "--cupy",
    "-c",
    action="store_true",
    dest="cupy",
    help="Use cupy to allocate CUDA for better performance (default: False)",
)
...
if args.benchmark:
    if os.path.isfile(
        os.path.join(inputpath, args.output, infilename + "_benchmark.txt")
    ):
...
if args.verbose:
    ...
    print(
        "Output:      ",
        os.path.join(inputpath, args.output, infilename + "_output.png"),
    )
...
with open(
    os.path.join(inputpath, args.output, infilename + "_arguments.txt"), "w"
) as arguments:
...
class Trafo:
    ...
    def __extend_img(self):
        ...
        img_extended = self.create_extenisons(
            lr_mir, tb_mir, corner_mir, width_diff, height_diff
        )

        if args.debug:
            img_extended.save(
                os.path.join(inputpath, args.output, infilename + "_img_extended.png")
            )
        return img_extended
    ...
    def __write_to_txtfile(self, y8x8, cb8x8, cr8x8):
        ...
        with open(
            os.path.join(inputpath, args.output, "debug", infilename + "_y.txt"), "w"
        ) as y_txt:
            with open(
                os.path.join(inputpath, args.output, "debug", infilename + "_cb.txt"),
                "w",
            ) as cb_txt:
                with open(
                    os.path.join(
                        inputpath, args.output, "debug", infilename + "_cr.txt"
                    ),
                    "w",
                ) as cr_txt:
    ...
    def rgb2ycbcr(self):
        ...
        if args.entropy:
            with open(
                os.path.join(inputpath, args.output, infilename + "_entropy.txt"), "w"
            ) as entropy_txt:
    ...
    def save_ycbcr_channels(self, y, cb, cr):
        ...
        imgcb.save(
            os.path.join(inputpath, args.output, "debug", infilename + "_cb.png")
        )
        imgcr.save(
            os.path.join(inputpath, args.output, "debug", infilename + "_cr.png")
        )
    ...
    def ycbcr2rgb(self, y=None, cb=None, cr=None):
        ...
        if args.debug:
            crop_recomb.save(
                os.path.join(
                    inputpath, args.output, "debug", infilename + "_rücktrafo.png"
                )
            )
...
class DCT:
    ...
    def __write_to_txtfileFDCT(self, y8x8, cb8x8, cr8x8):
        ...
        if args.cupy:
            ...
            cr8x8 = np.asnumpy(cr8x8)
        with open(
            os.path.join(inputpath, args.output, "debug", infilename + "_FDCT_y.txt"),
            "w",
        ) as FDCT_y_txt:
            with open(
                os.path.join(
                    inputpath, args.output, "debug", infilename + "_FDCT_cb.txt"
                ),
                "w",
            ) as FDCT_cb_txt:
                with open(
                    os.path.join(
                        inputpath, args.output, "debug", infilename + "_FDCT_cr.txt"
                    ),
                    "w",
                ) as FDCT_cr_txt:
                    for i, block in enumerate(y8x8):
    ...
    def __write_to_txtfileIDCT(self, y8x8, cb8x8, cr8x8):
        ...
        if args.cupy:
            ...
            cr8x8 = np.asnumpy(cr8x8)
        with open(
            os.path.join(inputpath, args.output, "debug", infilename + "_IDCT_y.txt"),
            "w",
        ) as IDCT_y_txt:
            with open(
                os.path.join(
                    inputpath, args.output, "debug", infilename + "_IDCT_cb.txt"
                ),
                "w",
            ) as IDCT_cb_txt:
                with open(
                    os.path.join(
                        inputpath, args.output, "debug", infilename + "_IDCT_cr.txt"
                    ),
                    "w",
                ) as IDCT_cr_txt:
                    for i, block in enumerate(y8x8):
    ...
    def execute_DCT(self, forward):
        ...
        if forward == True:
            ...
            if args.entropy:
                with open(
                    os.path.join(inputpath, args.output, infilename + "_entropy.txt"),
                    "a",
                ) as entropy_txt:
                    entropy_txt.write("Entropy after FDCT".center(48, "*") + "\n")
                print("- Entropy after FDCT but before Quantization")
                Entropy.execute_entropy(
                    self, yDCT, cbDCT, crDCT, rgb=False, pixels=False
                )
        else:
...
class Quantization:
    ...
    def __init__(self, imgs8x8, quality, infilename):
        ...
        if args.debug:
            self.__write_qtables_to_textfile(
                self.quality_qtable_y, self.quality_qtable_c
            )
    ...
    def __write_to_txtfile(self, y8x8, cb8x8, cr8x8):
        ...
        with open(
            os.path.join(
                inputpath, args.output, "debug", infilename + "_quantization_y.txt"
            ),
            "w",
        ) as y_txt:
            with open(
                os.path.join(
                    inputpath, args.output, "debug", infilename + "_quantization_cb.txt"
                ),
                "w",
            ) as cb_txt:
                with open(
                    os.path.join(
                        ...
                        args.output,
                        "debug",
                        infilename + "_quantization_cr.txt",
                    ),
                    "w",
                ) as cr_txt:
                    for i, block in enumerate(y8x8):
    ...
    def __write_qtables_to_textfile(self, qtable_y, qtable_c):
        ...
        with open(
            os.path.join(
                inputpath, args.output, "debug", infilename + "_Quantizationtable_y.txt"
            ),
            "w",
        ) as qtable_y_txt:
            with open(
                os.path.join(
                    ...
                    args.output,
                    "debug",
                    infilename + "_Quantizationtable_c.txt",
                ),
                "w",
            ) as qtable_c_txt:
                for i in range(8):
        ...
        if forward:
            ...
            if args.entropy:
                with open(
                    ...
                    entropy_txt.write(
                        "Entropy after quantization".center(48, "*") + "\n"
                    )
...
def subtract_images(img_input, img_output, img_height, img_width):
    ...
    if args.cupy:
        ...
        diff_img_multiplied = Image.fromarray(
            np.asnumpy(diff_multiplied).astype(np.uint8), "RGB"
        )
    else:
...
def trafo_dct_q():
    ...
    diff_img, diff_img_multiplied = subtract_images(
        rgb_input, rgb_output, img_height, img_width
    )
    if args.debug:
        diff_img.save(
            os.path.join(
                inputpath, args.output, "debug", infilename + "_diff_img_original.png"
            )
        )
    if args.psnr:
        noise_r, noise_g, noise_b, min_noise_r, min_noise_g, min_noise_b = compute_psnr(
            diff_img
        )
        if os.path.isfile(
